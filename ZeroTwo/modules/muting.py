...
from ZeroTwo import LOGGER, REAPERS, dispatcher
...
def check_user(user_id: int, bot: Bot, chat: Chat) -> Optional[str]:
    ...
    try:
        ...
        if excp.message == "User not found ree! check again":
            reply = "User not found!"
            return reply
        raise
    ...
    if is_user_admin(chat, user_id, member) or user_id in REAPERS:
...
def mute(update: Update, context: CallbackContext) -> str:
    ...
    if member.can_send_messages is None or member.can_send_messages:
        ...
        msg = f"{mention_html(member.user.id, member.user.first_name)} [<code>{member.user.id}</code>] is now muted in the chat."
        if reason:
        ...
        return log
    message.reply_text("Already muted!")
...
def unmute(update: Update, context: CallbackContext) -> str:
    ...
    if not user_id:
        message.reply_text(
            "Give argument or reply to the person to unmute."
        )
        return ""
    ...
    if member.status in ("kicked", "left"):
        ...
        bot.sendMessage(
            chat.id,
            "{} [<code>{}</code>] is now unmuted.".format(
    ...
    if match:
        ...
        if unmuted:
            update.effective_message.edit_text(
                f"{mention_html(member.user.id, member.user.first_name)} [<code>{member.user.id}</code>] is now unmuted.",
                parse_mode=ParseMode.HTML,
            )
