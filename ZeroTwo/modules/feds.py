...
import ZeroTwo.modules.sql.feds_sql as sql
from ZeroTwo import (
    EVENT_LOGS,
    LOGGER,
    SUPPORT_CHAT,
...
def leave_fed(update: Update, context: CallbackContext):
    ...
    if getuser in "creator" or user.id in SUDOS:
...
def user_join_fed(update: Update, context: CallbackContext):
    ...
    if is_user_fed_owner(fed_id, user.id) or user.id in SUDOS:
            ...
            with BytesIO() as file:
                ...
                for x in splitting:
                    ...
                    if str(import_userid) == str(OWNER_ID):
                        failed += 1
                        continue
                    if int(import_userid) in SUDOS:
                        failed += 1
                        continue
                    if int(import_userid) in REAPERS:
                        failed += 1
                        continue
                    if int(import_userid) in HELLHOUND:
                        failed += 1
                        continue
