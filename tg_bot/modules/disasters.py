...
def addsudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DEMONS:
        rt += "Succesfully raised Advisor to Vice President."
        data["supports"].remove(user_id)
        DEMONS.remove(user_id)

    if user_id in WOLVES:
        rt += "Succesfully Secretary raised  to Advisor."
    ...
    update.effective_message.reply_text(
        rt
        + "\nSuccessfully raised {} to Vice President!".format(
...
def addsupport(
    ...
    update.effective_message.reply_text(
        rt + f"\n{user_member.first_name} was added as a Advisor!",
    )
...
def addwhitelist(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        rt += "This member is a Vice President, Demoting to Advisor."
        data["sudos"].remove(user_id)
        DRAGONS.remove(user_id)

    if user_id in DEMONS:
        rt += "This user is already a Advisor, Demoting to Secretary."
        data["supports"].remove(user_id)
        DEMONS.remove(user_id)

    if user_id in WOLVES:
        message.reply_text("This user is already in Secretary.")
        return ""
    ...
    update.effective_message.reply_text(
        rt + f"\nSuccessfully raised {user_member.first_name} to be a Secretary!",
    )
...
def addtiger(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in WOLVES:
        rt += "This user is already a Secretary, Demoting him."
        data["whitelists"].remove(user_id)
        WOLVES.remove(user_id)

    if user_id in REAPERS:
        message.reply_text("This user is already at the lowest disaster.")
        return ""

    data["REAPERS"].append(user_id)
...
def removesudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        message.reply_text("Contacted support to demote this user to Civilian")
        DRAGONS.remove(user_id)
        data["sudos"].remove(user_id)
        ...
        return log_message
    message.reply_text("This user is not a Vice President!")
    return ""
...
def removesupport(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DEMONS:
        ...
        return log_message
    message.reply_text("This user is not a Advisor!")
    return ""
...
def removetiger(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in REAPERS:
        ...
        return log_message
    message.reply_text("This user is not a low level disaster!")
    return ""


@whitelist_plus
def whitelistlist(update: Update, context: CallbackContext):
    reply = "<b>Low level disasters :</b>\n"
    m = update.effective_message.reply_text(
        "<code>Contacting Void for intel..</code>",
...
def sudolist(update: Update, context: CallbackContext):
    ...
    true_sudo = list(set(DRAGONS) - set(MOD_USERS))
    reply = "<b>The Advisor:</b>\n"
