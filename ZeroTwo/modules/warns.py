...
import telegram
from ZeroTwo import REAPERS, HELLHOUND, dispatcher
...
def warn(
    ...
    if user.id in REAPERS:
        if warner:
            message.reply_text("Reapers cant be warned.")
        else:
            message.reply_text(
                "Reapers triggered an auto warn filter!\n I can't warn Reapers but they should avoid abusing this."
            )
        return
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
                f"Has disabled strong punches. I will use normal kick on users."
            )
