...
from ZeroTwo.modules.helper_funcs.msg_types import get_welcome_type
        ...
        if sibylClient: # if no client then does_chat_sibylban mostly won't be imported
            if does_chat_sibylban(chat.id):
                ...
                if data and data.banned:
                        return # all modes handle it in different way
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                ...
                welcome_log = (
                    ...
                    f"#USER_JOINED\n"
                    f"Watchers presence has been detected!"
                )
                continue

            # Welcome Devs
            if new_mem.id in MOD_USERS:
                update.effective_message.reply_text(
                    "Moderator User joined the chat!",
                    reply_to_message_id=reply,
                )
                continue

            # Welcome Sudos
            if new_mem.id in SUDOS:
                update.effective_message.reply_text(
                    "Sudo's Aura has been detected!",
                    reply_to_message_id=reply,
                )
                continue
