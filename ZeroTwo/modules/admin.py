...
from ZeroTwo import SUDOS, dispatcher
...
from ZeroTwo.modules.helper_funcs.admin_rights import (
    user_can_changeinfo,
    user_can_promote,
)
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    if user_member.status in ("administrator", "creator"):
        message.reply_text("User Status already admin!")
        return

    if user_id == bot.id:
        message.reply_text("Admin Status has to be appointed by another user!")
        return
    ...
    try:
        ...
        if err.message == "User_not_mutual_contact":
            message.reply_text("User Presence: Not in Group")
        else:
...
RP="https://telegra.ph/file/8dff0b1fcc9e2201dfa88.png"
...
def lowpromote(update: Update, context: CallbackContext) -> str:
    ...
    if (
        ...
        message.reply_photo(RP, caption="You don't have the necessary rights to do that!")
        return
    ...
    if not user_id:
        message.reply_text(
            "User not in Database or the information is incorrect!",
        )
        return
    ...
    try:
        ...
        if err.message == "User_not_mutual_contact":
            message.reply_text("User presence not detected.")
        else:
            message.reply_text("An error occured while promoting.")
        return
    ...
    bot.sendMessage(
        chat.id,
        f"Removing rights of a user in <b>{chat.title}<b>\n\nUser: {mention_html(user_member.user.id, user_member.user.first_name)}\nAdmin: {mention_html(user.id, user.first_name)}",
        parse_mode=ParseMode.HTML,
    )
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    if user_member.status == "creator":
        message.reply_text("Owners can't be demoted.")
        return

    if not user_member.status == "administrator":
        message.reply_text("User in NOT an admin.")
        return

    if user_id == bot.id:
        message.reply_text("Demote me manually.")
        return
...
def set_title(update: Update, context: CallbackContext):
    ...
    if user_member.status != "administrator":
        message.reply_text(
            "Non admins can't have custom title.",
        )
        return

    if user_id == bot.id:
        message.reply_text(
            "Admin has to do that for me!",
        )
        return
...
FULLPROMOTE_HANDLER = DisableAbleCommandHandler(
    ["fullpromote", "fpromote"], fullpromote, run_async=True
)
...
ADMIN_REFRESH_HANDLER = CommandHandler(
    ["reload", "admincache"], refresh_admin, filters=Filters.chat_type.groups, run_async=True
)
