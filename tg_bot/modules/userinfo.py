...
from tg_bot.modules.sql.afk_sql import is_afk, set_afk
...
@telethn.on(
    events.NewMessage(
        pattern="/ginfo ",
        from_users=(REAPERS or []) + (DRAGONS or []) + (DEMONS or []),
    ),
)
...
def info(update: Update, context: CallbackContext):
    ...
    rep = message.reply_text("<code>Accessing FranXX Database...</code>", parse_mode=ParseMode.HTML)
...
def set_about_me(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if repl_user_id in [bot.id, 777000, 1087968824] and (user_id in MOD_USERS):
...
def stats(update: Update, context: CallbackContext):
    ...
    update.effective_message.reply_message(
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id in [777000, 1087968824] and sender_id not in MOD_USERS:
            message.reply_text("You are not authorised")
            return

        if user_id == bot.id and sender_id not in MOD_USERS:
            message.reply_text(
                "Moderators are allowed to set my bio!",
            )
            return
